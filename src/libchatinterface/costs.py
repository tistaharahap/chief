    ...
    if model_name:
        ...
        if not model_costs:
            for pricing_key, pricing in MODEL_PRICING.items():
                if (normalized_name in pricing_key or 
                    pricing_key in normalized_name or
                    any(part in pricing_key for part in normalized_name.split('-')[:2])):
                    model_costs = pricing
                    break
...
def format_session_costs_for_metadata(session_costs: SessionCosts) -> Dict:
    ...
    result = {
        "total": format_usage_costs(session_costs.total_usage)
    }
