...
class SessionManager:
    ...
    def __init__(self, app_name: str = "chatinterface"):
        ...
        # Session tracking
        self.first_user_message: Optional[str] = None
    ...
    async def _extract_title(self, message: str) -> str:
        ...
        Returns:
            A concise, meaningful title (≤5 words) generated by Claude 3.5 Haiku
        """
        try:
            # Import here to use the same provider configuration as the main agent
            from libagentic.providers import get_default_model
            from pydantic_ai.settings import ModelSettings
            
            # Create a lightweight agent specifically for title generation using the same provider
            title_agent = Agent(
                get_default_model(anthropic_model_name="claude-3-5-haiku-latest", openai_model_name="gpt-4o"),
                model_settings=ModelSettings(temperature=0.1),  # Low temperature for consistent titles
                system_prompt="""You are a title generator. Create concise, meaningful titles for chat sessions.
...
Respond with ONLY the title, nothing else."""
            ...
            # Generate title using the agent - use result.output not result.data
    ...
    def _update_metadata(self, title: Optional[str] = None) -> None:
